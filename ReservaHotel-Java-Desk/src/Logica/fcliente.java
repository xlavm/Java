...
public class fcliente {
    ...
    public boolean insertar(vcliente dts) {
        /* primero debemos almacenar datos a la tabla persona y luego tendremos que
        ...
        */
        sSQL = "insert into persona (nombre,apaterno,amaterno,tipo_documento,num_documento,direccion,telefono,email)"
                + "values (?,?,?,?,?,?,?,?)";
        /*en la tabla cliente se almacena id de la persona el cual se consulta y se dice que coja el ultimo 
        agregado a la bd (desc limit 1)(el que se acabó de guardar con la instrucción anterior)
        luego almacenamos el codigo del cliente
        */
        sSQL2 = "insert into cliente (idpersona,codigo_cliente)"
                + "values ((select idpersona from persona order by idpersona desc limit 1),?)";
        try {
            PreparedStatement pst = cn.prepareStatement(sSQL);//para la consulta 1
            PreparedStatement pst2 = cn.prepareStatement(sSQL2);//para la consulta 2
            ...
            //la siguiente linea es para la consulta 2 
            pst2.setString(1, dts.getCodigo_cliente());

            int n = pst.executeUpdate();//ejecutamos el primer statement (pst)
            if (n != 0) {
                int n2 = pst2.executeUpdate();//ejecutamos el segundo statement (pst2)
                if (n2 != 0) {
                    return true;
                } else {
                    return false;
